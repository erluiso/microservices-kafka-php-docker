<?php
    ...
    class User
    {
        ...
        public function saveUser($name,$email)
        {
            try
            {
                $this->log->save("Saving new user: ".$name.", ".$email);
                $this->db->insert("INSERT INTO users (name,email) VALUES ('".$name."','".$email."')");
                $this->log->save("User saved!");
            }
            catch(Exception $e)
            {
                $this->log->save("Error save new user: ".$name.", ".$email);
                throw $e;
            }
        }
